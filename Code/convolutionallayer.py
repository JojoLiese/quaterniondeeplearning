...
def quatmaxpool(input, size, stride=1):
    output = torch.zeros(len(input[:,0,0,0,0]),len(input[0,:,0,0,0]),3,math.floor((len(input[0,0,0,:,0])-size)/stride)+1,math.floor((len(input[0,0,0,:,0])-size)/stride)+1)
    amplitudes = torch.zeros(size,size)
        ...
        for c in range(len(input[0,:,0,0,0])):
            for i in range(math.floor((len(input[0,0,0,:,0])-size)/stride)+1):
                for j in range(math.floor((len(input[0, 0, 0, :, 0]) - size) / stride) + 1):
                    ...
                    x,y = np.unravel_index(maxcount, (size, size))
